...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        run_command(
            [@$ssh_cmd, $qnetd_addr, $qnetd_certutil, "-i"], noerr => 1,
        );
        ...
        $foreach_member->(sub {
            my ($node, $ip) = @_;
            my $ssh_cluster_cmd =
                PVE::SSHInfo::ssh_info_to_command({ ip => $ip, name => $node });
            my $outsub = sub { print "\nnode '$node': " . shift };
            run_command(
                [
                    @$ssh_cluster_cmd,
...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        die
            "Navigate out of $basedir before running 'pvecm add', for example by running 'cd'.\n"
            if getcwd() =~ m!^$basedir(/.*)?$!;
        ...
        my $worker = sub {
            ...
            if (!$param->{use_ssh}) {
                my $password =
                    PVE::PTY::read_password("Please enter superuser (root) password for '$host': ");
                ...
                PVE::Tools::lock_file(
                    $local_cluster_lock, 10, \&PVE::Cluster::Setup::join, $param,
                );

                if (my $err = $@) {
                    if (
                        ref($err) eq 'PVE::APIClient::Exception'
                        && defined($err->{code})
                        && $err->{code} == 501
                    ) {
            ...
            $cmd =
                ['ssh', $host, '-o', 'BatchMode=yes', 'pvecm', 'addnode', $nodename, '--force', 1];
            ...
            foreach my $link (keys %$links) {
                push @$cmd, "--link$link",
                    PVE::JSONSchema::print_property_string($links->{$link}, $link_desc->{format});
            }
            ...
            eval {
                ...
                $cmd = [
                    'rsync',
                    '--rsh=ssh -l root -o BatchMode=yes',
                    '-lpgoq',
                    "[$host]:$authfile",
                    "[$host]:$clusterconf",
                    $tmpdir,
...
__PACKAGE__->register_method({
    ...
    code => sub {
        my ($param) = @_;
        my ($force_new_cert, $silent, $unmerge) =
            $param->@{qw(force silent unmerge-known-hosts)};
    ...
    code => sub {
        ...
        my $get_local_migration_ip = sub {
            ...
            if (defined($cidr)) {
                ...
                die
                    "could not get migration ip: multiple, different, IP address configured for "
                    . "network '$cidr'\n"
