...
static void dfsm_send_sync_message_abort(dfsm_t *dfsm) {
...
static void
dfsm_record_local_result(dfsm_t *dfsm, uint64_t msg_count, int msg_result, gboolean processed) {
    g_return_if_fail(dfsm != NULL);
...
static cs_error_t
dfsm_send_message_full(dfsm_t *dfsm, struct iovec *iov, unsigned int len, int retry) {
...
static cs_error_t
dfsm_send_state_message_full(dfsm_t *dfsm, uint16_t type, struct iovec *iov, unsigned int len) {
    g_return_val_if_fail(dfsm != NULL, CS_ERR_INVALID_PARAM);
...
static gboolean dfsm_send_checksum(dfsm_t *dfsm) {
    g_return_val_if_fail(dfsm != NULL, FALSE);
...
static void dfsm_free_queue_entry(gpointer data) {
    dfsm_queued_message_t *qm = (dfsm_queued_message_t *)data;
    g_free(qm->msg);
    g_free(qm);
}

static void dfsm_free_message_queue(dfsm_t *dfsm) {
    g_return_if_fail(dfsm != NULL);
    ...
    while (iter != end) {
        ...
        dfsm_queued_message_t *qm = (dfsm_queued_message_t *)g_sequence_get(cur);
        dfsm_free_queue_entry(qm);
        g_sequence_remove(cur);
    }
}

static void dfsm_free_sync_queue(dfsm_t *dfsm) {
    g_return_if_fail(dfsm != NULL);
...
static gint message_queue_sort_fn(gconstpointer a, gconstpointer b, gpointer user_data) {
    return ((dfsm_queued_message_t *)a)->msg_count - ((dfsm_queued_message_t *)b)->msg_count;
}

static dfsm_node_info_t *dfsm_node_info_lookup(dfsm_t *dfsm, uint32_t nodeid, uint32_t pid) {
...
static dfsm_queued_message_t *dfsm_queue_add_message(
    dfsm_t *dfsm, uint32_t nodeid, uint32_t pid, uint64_t msg_count, const void *msg, size_t msg_len
) {
    g_return_val_if_fail(dfsm != NULL, NULL);
...
static guint dfsm_sync_info_hash(gconstpointer key) {
    dfsm_node_info_t *info = (dfsm_node_info_t *)key;

    return g_int_hash(&info->nodeid) + g_int_hash(&info->pid);
}

static gboolean dfsm_sync_info_equal(gconstpointer v1, gconstpointer v2) {
...
static int dfsm_sync_info_compare(gconstpointer v1, gconstpointer v2) {
    dfsm_node_info_t *info1 = (dfsm_node_info_t *)v1;
    dfsm_node_info_t *info2 = (dfsm_node_info_t *)v2;
...
static void dfsm_set_mode(dfsm_t *dfsm, dfsm_mode_t new_mode) {
    g_return_if_fail(dfsm != NULL);
...
static dfsm_mode_t dfsm_get_mode(dfsm_t *dfsm) {
...
gboolean dfsm_restartable(dfsm_t *dfsm) {
    ...
    return !(mode == DFSM_MODE_ERROR || mode == DFSM_MODE_VERSION_ERROR);
}

void dfsm_set_errormode(dfsm_t *dfsm) { dfsm_set_mode(dfsm, DFSM_MODE_ERROR); }

static void dfsm_release_sync_resources(
    dfsm_t *dfsm, const struct cpg_address *member_list, size_t member_list_entries
) {
    g_return_if_fail(dfsm != NULL);
    ...
    if (member_list) {
        ...
        qsort(
            sync_info->nodes, member_list_entries, sizeof(dfsm_node_info_t), dfsm_sync_info_compare
        );
...
static void dfsm_cpg_deliver_callback(
    ...
    if (base_header->protocol_version > dfsm->protocol_version) {
        cfs_dom_critical(
            dfsm->log_domain, "received message with protocol version %d",
        ...
        cfs_dom_message(
            dfsm->log_domain, "ignore message with wrong protocol version %d",
            base_header->protocol_version
        );
        return;
    }
    ...
    if (base_header->type == DFSM_MESSAGE_NORMAL) {
        ...
        if (msg_len < sizeof(dfsm_message_normal_header_t)) {
            cfs_dom_critical(
                dfsm->log_domain, "received short message (type = %d, subtype = %d, %zd bytes)",
        ...
        if (mode != DFSM_MODE_SYNCED) {
            cfs_dom_debug(
                dfsm->log_domain, "queue message %" PRIu64 " (subtype = %d, length = %zd)",
            ...
            int res = dfsm->dfsm_callbacks->dfsm_deliver_fn(
                dfsm, dfsm->data, &msg_res, nodeid, pid, base_header->subtype, base_header->time,
                (uint8_t *)msg + sizeof(dfsm_message_normal_header_t),
    ...
    if (msg_len < sizeof(dfsm_message_state_header_t)) {
        cfs_dom_critical(
            dfsm->log_domain, "received short state message (type = %d, subtype = %d, %zd bytes)",
            base_header->type, base_header->subtype, msg_len
        );
        goto leave;
    }
    ...
    if (base_header->type != DFSM_MESSAGE_SYNC_START &&
        ...
        cfs_dom_debug(
            ...
            "wrong epoch (epoch %d/%d/%08X)",
            base_header->type, header->epoch.nodeid, header->epoch.pid, header->epoch.epoch
        );
        return;
    }
    ...
    if (mode == DFSM_MODE_SYNCED) {
        if (base_header->type == DFSM_MESSAGE_UPDATE_COMPLETE) {
            ...
            if (msg_len != sizeof(dfsm->csum_counter)) {
                cfs_dom_critical(
                    dfsm->log_domain,
                    "cpg received verify request with wrong length (%zd bytes) form node %d/%d",
                    msg_len, nodeid, pid
                );
                goto leave;
            }
            ...
            cfs_dom_debug(
                dfsm->log_domain, "got verify request from node %d %016" PRIX64, nodeid, csum_id
            );
            ...
            if (dfsm->dfsm_callbacks->dfsm_checksum_fn) {
                ...
                if (dfsm->csum_id == csum_id &&
                    ...
                    if (memcmp(msg, dfsm->csum, sizeof(dfsm->csum)) != 0) {
                        cfs_dom_critical(
                            dfsm->log_domain,
                            "wrong checksum %016" PRIX64 " != %016" PRIX64 " - restarting",
            ...
            cfs_dom_debug(dfsm->log_domain, "ignore state sync message %d", base_header->type);
            return;
        }
        ...
        if (base_header->type == DFSM_MESSAGE_SYNC_START) {
            ...
            if (nodeid != dfsm->lowest_nodeid) {
                cfs_dom_critical(
                    dfsm->log_domain, "ignore sync request from wrong member %d/%d", nodeid, pid
                );
            }

            cfs_dom_message(
                dfsm->log_domain, "received sync request (epoch %d/%d/%08X)", header->epoch.nodeid,
                header->epoch.pid, header->epoch.epoch
            );
            ...
            if (!(ni = dfsm_node_info_lookup(dfsm, nodeid, pid))) {
                cfs_dom_critical(
                    dfsm->log_domain, "received state for non-member %d/%d", nodeid, pid
                );
                goto leave;
            }
            ...
            if (ni->state) {
                cfs_dom_critical(
                    dfsm->log_domain, "received duplicate state for member %d/%d", nodeid, pid
                );
                goto leave;
            }
    ...
    if (base_header->type == DFSM_MESSAGE_VERIFY_REQUEST ||
        ...
        cfs_dom_debug(
            dfsm->log_domain, "ignore verify message %d while not synced", base_header->type
        );

    } else {
        cfs_dom_critical(
            dfsm->log_domain, "received unknown state message type (type = %d, %zd bytes)",
...
static gboolean dfsm_resend_queue(dfsm_t *dfsm) {
    g_return_val_if_fail(dfsm != NULL, FALSE);
...
static gboolean dfsm_deliver_sync_queue(dfsm_t *dfsm) {
    ...
    // fixme: cfs_debug
    cfs_dom_message(
        dfsm->log_domain, "%s: queue length %d", __func__, g_list_length(dfsm->sync_queue)
    );
    ...
    while (iter) {
        ...
        if (res && dfsm->mode == DFSM_MODE_SYNCED) {
            dfsm_cpg_deliver_callback(
                dfsm->cpg_handle, &dfsm->cpg_group_name, qm->nodeid, qm->pid, qm->msg, qm->msg_len
            );
        } else {
            res = FALSE;
        }
...
static gboolean dfsm_deliver_queue(dfsm_t *dfsm) {
    ...
    while (iter != end) {
        ...
        if (!ni) {
            cfs_dom_message(
                dfsm->log_domain, "remove message from non-member %d/%d", qm->nodeid, qm->pid
            );
            dfsm_free_queue_entry(qm);
            g_sequence_remove(cur);
            continue;
        ...
        if (dfsm->mode == DFSM_MODE_SYNCED) {
            if (ni->synced) {
                dfsm_cpg_deliver_callback(
                    dfsm->cpg_handle, &dfsm->cpg_group_name, qm->nodeid, qm->pid, qm->msg,
                    qm->msg_len
                );
                dfsm_free_queue_entry(qm);
                g_sequence_remove(cur);
...
static void dfsm_cpg_confchg_callback(
    cpg_handle_t handle,
    const struct cpg_name *group_name,
    ...
    */
    ...
    if (!dfsm->we_are_member) {
        ...
        cfs_dom_message(
            dfsm->log_domain, "we (%d/%d) left the process group", dfsm->nodeid, dfsm->pid
        );
        goto leave;
    }
...
static gboolean service_dfsm_finalize(cfs_service_t *service, gpointer context) {
...
static int service_dfsm_initialize(cfs_service_t *service, gpointer context) {
...
static gboolean service_dfsm_dispatch(cfs_service_t *service, gpointer context) {
    g_return_val_if_fail(service != NULL, FALSE);
    g_return_val_if_fail(context != NULL, FALSE);
...
static void service_dfsm_timer(cfs_service_t *service, gpointer context) {
...
void service_dfsm_destroy(cfs_service_t *service) {
    ...
    service_dfsm_private_t *private = (service_dfsm_private_t *)cfs_service_get_context(service);
