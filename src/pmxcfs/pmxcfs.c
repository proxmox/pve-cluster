...
static void glib_print_handler(const gchar *string) { printf("%s", string); }
...
void *cfs_fuse_init(struct fuse_conn_info *conn) { return NULL; }
...
static int
cfs_fuse_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    (void)fi;
...
static char *create_dot_version_cb(cfs_plug_t *plug) {
...
static char *create_dot_members_cb(cfs_plug_t *plug) {
    GString *outbuf = g_string_new(NULL);
    char *data = NULL;
...
static char *create_dot_vmlist_cb(cfs_plug_t *plug) {
    GString *outbuf = g_string_new(NULL);
    char *data = NULL;
...
static char *create_dot_rrd_cb(cfs_plug_t *plug) {
    GString *outbuf = g_string_new(NULL);
...
static void my_qb_log_filter(struct qb_log_callsite *cs) {
...
static void update_qb_log_settings(void) {
...
static const char *log_tags_stringify(uint32_t tags) {
...
int main(int argc, char *argv[]) {
    ...
    cfs_message(
        "resolved node name '%s' to '%s' for default node IP address", cfs.nodename, cfs.ip
    );
