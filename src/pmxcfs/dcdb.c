...
static gboolean
dcdb_parse_unlock_request(const void *msg, size_t msg_len, const char **path, const guchar **csum)
...
static gboolean dcdb_parse_fuse_message(
    const void *msg,
    size_t msg_len,
    const char **path,
    const char **to,
...
static gboolean dcdb_send_update_inode(dfsm_t *dfsm, memdb_tree_entry_t *te) {
...
memdb_tree_entry_t *dcdb_parse_update_inode(const void *msg, size_t msg_len) {
    if (msg_len < 40) {
...
void dcdb_sync_corosync_conf(memdb_t *memdb, gboolean notify_corosync) {
    ...
    if (!g_file_get_contents(HOST_CLUSTER_CONF_FN, &old_data, &old_length, &err)) {
        if (!g_error_matches(err, G_FILE_ERROR, G_FILE_ERROR_NOENT)) {
            cfs_critical(
                "unable to read cluster config file '%s' - %s", HOST_CLUSTER_CONF_FN, err->message
            );
        }
        g_error_free(err);
    } else {
...
static gpointer dcdb_get_state(dfsm_t *dfsm, gpointer data, unsigned int *res_len) {
    ...
    cfs_debug(
        "enter %s %016" PRIX64 " %08X", __func__, (uint64_t)memdb->root->version, memdb->root->mtime
    );
...
static int dcdb_select_leader(int node_count, memdb_index_t *idx[]) {
    ...
    for (int i = 0; i < node_count; i++) {
        if (leader < 0) {
            ...
            if (idx[i]->version == leaderidx->version && idx[i]->mtime > leaderidx->mtime) {
                leader = i;
...
static gboolean dcdb_create_and_send_updates(
    dfsm_t *dfsm, memdb_t *memdb, memdb_index_t *master, int node_count, memdb_index_t *idx[]
) {
    g_return_val_if_fail(dfsm != NULL, FALSE);
...
static int dcdb_process_state_update(dfsm_t *dfsm, gpointer data, dfsm_sync_info_t *syncinfo) {
    ...
    g_string_append_printf(
        synced_member_ids, "%d/%d", syncinfo->nodes[leader].nodeid, syncinfo->nodes[leader].pid
    );
...
static int dcdb_process_update(
    dfsm_t *dfsm,
    gpointer data,
...
static int dcdb_commit(dfsm_t *dfsm, gpointer data, dfsm_sync_info_t *syncinfo) {
    g_return_val_if_fail(dfsm != NULL, -1);
    g_return_val_if_fail(data != NULL, -1);
    g_return_val_if_fail(syncinfo != NULL, -1);
    g_return_val_if_fail(syncinfo->data != NULL, -1);
...
static int dcdb_cleanup(dfsm_t *dfsm, gpointer data, dfsm_sync_info_t *syncinfo) {
    g_return_val_if_fail(dfsm != NULL, -1);
    g_return_val_if_fail(data != NULL, -1);
    g_return_val_if_fail(syncinfo != NULL, -1);
    g_return_val_if_fail(syncinfo->data != NULL, -1);
...
gboolean dcdb_checksum(dfsm_t *dfsm, gpointer data, unsigned char *csum, size_t csum_len) {
    g_return_val_if_fail(dfsm != NULL, FALSE);
    g_return_val_if_fail(csum != NULL, FALSE);
...
static int dcdb_deliver(
    ...
    if (msg_type == DCDB_MESSAGE_CFS_UNLOCK_REQUEST || msg_type == DCDB_MESSAGE_CFS_UNLOCK) {
        ...
        if (te && te->type == DT_DIR && path_is_lockdir(path) &&
            memdb_tree_entry_csum(te, cur_csum) && (memcmp(csum, cur_csum, 32) == 0)) {
...
dfsm_t *dcdb_new(memdb_t *memdb) {
    ...
    return dfsm_new(
        memdb, DCDB_CPG_GROUP_NAME, G_LOG_DOMAIN, DCDB_PROTOCOL_VERSION, &dcdb_dfsm_callbacks
    );
}
