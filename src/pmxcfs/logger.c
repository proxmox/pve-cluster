...
static clog_entry_t *clog_alloc_entry(clog_base_t *clog, uint32_t size) {
...
static void clog_dump_entry(clog_entry_t *cur, uint32_t cpos) {
    ...
    printf(
        " %s{%016" PRIX64 "} %s[%s{%016" PRIX64 "}]: %s\n", node, cur->node_digest, tag, ident,
        cur->ident_digest, msg
    );
}
...
void clog_dump_json(clog_base_t *clog, GString *str, const char *ident, guint max_entries) {
    ...
    while (cpos && (cpos <= clog->cpos || cpos > (clog->cpos + CLOG_MAX_ENTRY_SIZE))) {
        ...
        g_string_append_printf(
            ...
            "\"pid\": %u, \"node\": \"%s\", \"user\": \"%s\", "
            "\"msg\": \"%s\"}",
            cur->uid, cur->time, cur->priority, tag, cur->pid, node, ident, msg
        );
...
uint32_t clog_entry_size(const clog_entry_t *entry) {
    ...
    return sizeof(clog_entry_t) + entry->node_len + entry->ident_len + entry->tag_len +
           entry->msg_len;
}
...
uint32_t clog_pack(
    ...
    *msg_start = 0;
    ...
    uint32_t size = sizeof(clog_entry_t) + node_len + ident_len + tag_len + msg_len;
...
static gint clog_entry_sort_fn(gconstpointer v1, gconstpointer v2, gpointer user_data) {
...
static gboolean clog_tree_foreach_fn(gpointer key, gpointer value, gpointer data) {
...
static gboolean dedup_lookup(GHashTable *dedup, const clog_entry_t *entry) {
